...
基础操作
命令	操作	备注
查看	ls -al path	ll 是个别名
新建目录	mkdir -p path	-p可递归创建,即便存在也不报错
当前目录	pwd
新建文件	vi path/file	:wq保存退出
...
执行脚本/sbin/init（读取/etc/inittab文件确定运行等级）
执行脚本/etc/rc.d/rc.sysinit，设定path等
根据等级3，执行/etc/rc3.d/目录下软链的服务（软链指向/etc/init.d/目录）
执行/etc/rc.d/rc.local脚本，可DIY shell命令
执行/bin/login触发登录
手动开关服务(了解即可)
...
不在服务里也需要开机启动，比如svn，可通过rc.local

vi /etc/rc.d/rc.local
/usr/bin/svnserve -d -r /home/svn (根据实际追加命令)
自启动服务
chkconfig 命令，帮服务在rc3.d里建软链，实现开机自启动：

chkconfig httpd on/off
查看已经自启动的服务
chkconfig --list
新命令systemctl整合了service和chkconfig

环境变量
已经安装的程序是可以更改目录的，但需确认：

依赖你的程序知道你换位置了
重启后如需自启动，init.d下的脚本知道你换位置了
之前创建的一些软链知道你换位置了
...
rpm -q --qf %{version} redhat-release-server;echo 6Server
...
secureCRT
该软件安装在windows下，美化操作如下

Session Options --> Terminal 中将Anti-idle下的 Sending String选项打钩，每15分钟发个1来防踢。
Session Options --> Terminal --> Emulation界面中Terminal下拉条选为Linux，勾选ANSI color
Session Options --> Terminal --> Appearance中，将Fonts下的Normal Font调整为Consolas 10pt， 下面的Character Encoding设置为UTF-8
【黑科技】可对连接点右键 --> 创建桌面快捷方式
服务器环境
nginx
配置流程稍微复杂，找maoda安装配置即可
...
解压即安装：官网 找到 node-vxx-linux-x64.tar.gz,解压到/usr/local/目录下

配置：

移动到/usr/local目录下，命名为nodejs

//配置软链，可全局使用node npm 两个命令
...
把上述命令放vi /etc/rc.d/rc.local里可开机自启

//设置软链，便于启动mongodb的shell
